...
# The `terrainr` R package

# Viewshed Analyses with `terrainr`

To illustrate how these features may be applied to visual resources stewardship,
we will walk through visualizing an example viewshed analysis using `terrainr` 
and Unity. As an example, we will examine the potential visual impact of the 
Johns Brook Lodge building, located within the Adirondack High Peaks, 
All the code involved in this process has been included as Figure 
\@ref(fig:code_required).

```{r code_required, fig.cap="All the R code required for the visualizations incorporated in this paper. In addition, viewshed calculation was done using GRASS GIS version 7.8, with the outputs saved as an image using QGIS. Descriptions of functions and their arguments is available online at https://docs.ropensci.org/terrainr/"}
